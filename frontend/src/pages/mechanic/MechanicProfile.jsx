...
// Assuming ProfileInput component is imported correctly
...
  }, [dispatch]);
  
  // Update editedMechanicData when the Redux profile state changes (initial load or subsequent successful updates)
  useEffect(() => {
    if (profile) {
      setEditedMechanicData({
        fullName: profile.full_name || '',
        email: profile.email || '', 
        phoneNumber: profile.role_details?.contact_number || '', // Use optional chaining for safety
        profilePicture: profile.profile_picture || null, 
      });
    }
  }, [profile]);

  // Handle Redux success/error notifications
  useEffect(() => {
    if (success) {
      showNotification(success, 'success');
    }
    if (error) {
      // Specific handling for error object from backend (like password errors) vs general string error
...
    if (isEditMode) {
      // Revert changes if cancelling edit mode
      if (profile) {
        setEditedMechanicData({
          fullName: profile.full_name || '',
          email: profile.email || '',
...
      setEditedMechanicData(prev => ({
        ...prev,
        profilePicture: URL.createObjectURL(file) // Use local URL for instant preview
      }));
    }
  };
...
      .unwrap()
      .then(() => {
        // FIX: Re-fetch profile data after successful update
        dispatch(getProfile()); 
        setIsEditMode(false);
        setProfilePictureFile(null);
      })
...
  // Ensure current status is treated as a string for comparison
  const currentStatus = profile.role_details.availability; 
  let newStatus;

  if (currentStatus && currentStatus.toUpperCase() === 'AVAILABLE') { 
    newStatus = 'BUSY'; 
  } else {
    newStatus = 'AVAILABLE'; 
  }
...
    .unwrap()
    .then(() => {
      // FIX: Re-fetch profile data after successful availability update
      dispatch(getProfile());
    })
    .catch((err) => {
...
        showNotification(passwordMatchError, 'error');
      } else {
        showNotification('Please fill in all password fields correctly.', 'error');
      }
      return;
    }
    
...
      .catch((err) => {
        if (!isPasswordFormValid) {
              // Optionally show an error if validation fails before dispatch
              if (passwordMatchError) {
                showNotification(passwordMatchError, 'error');
              } else {
                showNotification('Please fill in all password fields correctly.', 'error');
...
                <ProfileInput
                  label="Full Name"
                  // Use editedMechanicData in edit mode, otherwise use profile data
